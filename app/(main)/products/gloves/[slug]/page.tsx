...
export async function generateMetadata({ params }: ProductPageProps): Promise<Metadata> {
  ...
  return {
    title: `${product.name} | Safety Gloves | HandLine Company`,
...
export default async function GloveProductPage({ params }: ProductPageProps) {
  // Decode the slug to get the original product name
  const decodedSlug = decodeURIComponent(params.slug);
  const { product } = await getProductBySlug(decodedSlug);
  ...
  // Filter related products to glove category
  const { relatedProducts } = await getRelatedProducts(product.id);
  const gloveRelatedProducts = relatedProducts.filter(p => 
