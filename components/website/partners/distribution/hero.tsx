...
import { Building, Truck, Globe, Shield, Flag, ChevronRight, Mail, Package } from "lucide-react";
        ...
        <div className="flex flex-col items-center text-center max-w-4xl mx-auto">
          ...
          >
            <div className="inline-flex items-center rounded-full border border-brand-primary px-3 py-1 md:px-4 md:py-1.5 text-xs md:text-sm backdrop-blur-sm">
              <Truck className="mr-1.5 h-3 w-3 md:h-4 md:w-4 text-brand-primary" />
              <span className="text-brand-dark dark:text-white font-medium">
                {t("partners.distributionPage.hero.badge")}
              </span>
            </div>
          </motion.div>
          ...
          >
            <h1 className="text-3xl md:text-4xl lg:text-5xl xl:text-6xl font-bold tracking-tight text-brand-dark dark:text-white font-heading">
              {t("partners.distributionPage.hero.title")} <span className="text-brand-primary">Hand Line</span> {t("partners.distributionPage.hero.titleSuffix")}
            </h1>
          </motion.div>
          ...
          >
            <p className="max-w-2xl text-base md:text-lg text-brand-secondary dark:text-gray-300">
              {t("partners.distributionPage.hero.description")}
            </p>
          </motion.div>
          ...
          >
            <Button 
              ...
              asChild
            >
              <Link href="#distribution-form" className="flex items-center justify-center gap-1.5 md:gap-2">
                <Truck className="h-4 w-4 md:h-5 md:w-5" />
                <span>{t("partners.distributionPage.hero.becomeDistributor")}</span>
