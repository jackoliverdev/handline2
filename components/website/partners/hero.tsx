...
import { Building, Users, Clock, Shield, Flag, ChevronRight, Mail, Briefcase } from "lucide-react";
...
export const PartnersHero = () => {
  const { t } = useLanguage();
  const stats = [
    { value: '30+', label: t('partners.hero.stats.countries') },
    { value: '150+', label: t('partners.hero.stats.partners') },
    { value: '40+', label: t('partners.hero.stats.years') },
    { value: '200+', label: t('partners.hero.stats.products') },
  ];
  return (
        ...
        <div className="flex flex-col items-center text-center max-w-4xl mx-auto">
          ...
          >
            <Button 
              ...
              asChild
            >
              <Link href="/partners/partnerships" className="flex items-center justify-center gap-1.5 md:gap-2">
                <Building className="h-4 w-4 md:h-5 md:w-5" />
                <span>{t('partners.hero.partnerships')}</span>
            ...
            <Button 
              ...
              asChild
            >
              <Link href="/partners/distribution" className="flex items-center justify-center gap-1.5 md:gap-2">
                <Mail className="h-4 w-4 md:h-5 md:w-5" />
                <span>{t('partners.hero.distribution')}</span>
                <ChevronRight className="h-3.5 w-3.5 md:h-4 md:w-4 transition-transform duration-300 group-hover:translate-x-1" />
              </Link>
            </Button>
          </motion.div>
