...
export function EyeFaceEquipment({ product }: { product: Product }) {
  const { t, language } = useLanguage();
  const items = (product as any)?.eye_face_equipment_locales?.[language]
    || (product as any)?.eye_face_equipment_locales?.en
    || [];
  ...
  return (
    <div className="group relative overflow-hidden rounded-lg border bg-white dark:bg-black/50 shadow-sm transition-all duration-300 hover:shadow-md border-brand-primary/10 dark:border-brand-primary/20 backdrop-blur-sm p-4">
      ...
      <ul className="list-disc list-inside space-y-1 text-brand-secondary dark:text-gray-300">
        {items.map((entry: string, idx: number) => (
          <li key={idx}>{entry}</li>
        ))}
      </ul>
    </div>
