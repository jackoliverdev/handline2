...
export function HeadOtherDetails({ product }: { product: Product }) {
  const { t, language } = useLanguage();
  const locales: any = (product as any).head_other_details_locales || {};
  ...
  return (
    <div className="group relative overflow-hidden rounded-lg border bg-white dark:bg-black/50 shadow-sm transition-all duration-300 hover:shadow-md border-brand-primary/10 dark:border-brand-primary/20 backdrop-blur-sm p-4">
      <div className="flex items-center gap-2 mb-2">
        ...
        <h3 className="font-medium text-brand-dark dark:text-white">{t('productPage.otherDetails') || 'Other details'}</h3>
      </div>
