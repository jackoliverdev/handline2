...
export const GlovesHero = () => {
  const { t } = useLanguage();
  const stats = [
    { value: "350°C", label: t("glovesHero.stats.heat") },
    { value: t("glovesHero.stats.cutValue"), label: t("glovesHero.stats.cut") },
    { value: "CE", label: t("glovesHero.stats.certified") },
    { value: "40+", label: t("glovesHero.stats.years") },
  ];
        ...
        <div className="flex flex-col items-center text-center max-w-4xl mx-auto">
          ...
          >
            <h1 className="text-3xl md:text-4xl lg:text-5xl xl:text-6xl font-bold tracking-tight text-brand-dark dark:text-white font-heading">
              {t("glovesHero.heading")} <span className="text-brand-primary">{t("glovesHero.headingAccent")}</span>
            </h1>
          </motion.div>
          ...
          >
            <Button 
              ...
              asChild
            >
              <Link href="#glove-categories" className="flex items-center justify-center gap-1.5 md:gap-2">
                <ShoppingBag className="h-4 w-4 md:h-5 md:w-5" />
                <span>{t("glovesHero.browseProducts")}</span>
            ...
            >
              <Link href="/contact" className="flex items-center justify-center gap-1.5 md:gap-2">
                ...
                <span>{t("glovesHero.getInTouch")}</span>
                <ChevronRight className="h-3.5 w-3.5 md:h-4 md:w-4 transition-transform duration-300 group-hover:translate-x-1" />
              </Link>
            </Button>
          </motion.div>
