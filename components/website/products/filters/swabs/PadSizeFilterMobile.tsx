...
interface PadSizeFilterMobileProps {
  options: string[];
  selected: string[];
  onToggle: (option: string) => void;
}

export const PadSizeFilterMobile = ({ options = [], selected, onToggle }: PadSizeFilterMobileProps) => {
  const { t } = useLanguage();
    ...
    <div>
      ...
      {open && (
        <div className="space-y-2 max-h-40 overflow-y-auto pr-2 mt-2">
          {options.map((opt) => {
            const id = `m-pad-${opt.replace(/[^a-zA-Z0-9]/g, "-")}`;
            return (
